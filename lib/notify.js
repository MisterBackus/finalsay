...
export async function tweetReplaced(prev, next) {
  ...
  try {
    ...
    const site = (process.env.SITE_URL || "https://finalsay.lol").replace(/^https?:\/\//, "");
    const line1 = prev.paid_cents ? `Erased after ${lasted(prev.lasted_ms)}: "${prev.text}"` : `First sentence bought.`;
    const body = `${line1}\n\nNow (${dollars(next.paid_cents)}${next.hold ? ", locked 1h" : ""}): "${next.text}"\n\nReplace it for ${dollars(next.next_price)} → ${site}`;
